...
def display_app_info():
    ...
    with st.expander("ℹ️ How to Use This App"):
        ...
        **Key Features:**
        ...
        - Market-wide Detection: Alerts when multiple groups show coordinated shifts
...
def create_download_data(trend_shifts, market_analysis, groups_scanned):
    ...
    header_lines = [
        "# Enhanced Seasonality & Trend Shift Analysis Results",
        f"# Current Week: {current_week} ({datetime.now().strftime('%Y-%m-%d')})",
        ...
        f"# Groups with Trend Shifts: {len(trend_shifts)}",
        ""
    ]
    
    if market_analysis:
        # Add broad pattern analysis
        if market_analysis.get('broad_patterns') and market_analysis['broad_patterns'].get('patterns'):
            header_lines.append("# Broad Market Patterns:")
            for pattern in market_analysis['broad_patterns']['patterns'][:3]:
                header_lines.append(f"# - {pattern['pattern']}: {pattern['count']} groups ({pattern['percentage']:.1f}%)")
            header_lines.append("")
        
        # Add inflection point analysis
        if market_analysis.get('market_shifts'):
            header_lines.append("# Market-Wide Inflection Points:")
            for shift in market_analysis['market_shifts']:
                header_lines.append(f"# - {shift['type'].title()} Week {shift['week']}: {shift['group_count']} industries ({shift['percentage']:.1f}% of market)")
            header_lines.append("")
        
        # Add sentiment analysis
        if market_analysis.get('trend_consensus'):
            consensus = market_analysis['trend_consensus']
            header_lines.extend([
                "# Market Sentiment:",
                f"# - Bullish Trends: {consensus['bullish_trends']}",
                f"# - Bearish Trends: {consensus['bearish_trends']}",
                f"# - Mixed Signals: {consensus['mixed_signals']}",
                ""
            ])
...
def analyze_broad_trend_patterns(trend_shifts):
    """Analyze broad market trend patterns beyond just inflection points"""
    if not trend_shifts:
        return None
...
def analyze_market_wide_shifts(trend_shifts):
    ...
    return {
        ...
        'broad_patterns': trend_pattern_analysis
    }
...
def display_market_wide_alert(market_analysis):
    ...
    # Display broad market trend patterns
    if has_broad_patterns:
        ...
        st.markdown("**Widespread patterns affecting large portions of the market:**")
        
        for i, pattern in enumerate(broad_data['patterns'][:5], 1):
...
def display_summary_statistics(groups_scanned, trend_shifts, market_analysis):
    ...
    # Add download button prominently
...
def display_individual_sidebar_stats(results):
    ...
    st.sidebar.markdown("### Group Statistics")
...
def handle_individual_analysis(selected_group, view_type, num_years, symbols, sector_stocks, industry_stocks):
    ...
    # Add refresh button for individual analysis
    if use_cached:
        col1, col2 = st.columns([3, 1])
        with col1:
...
def display_scan_results():
    ...
    if trend_shifts:
        ...
        if market_analysis and (market_analysis['market_shifts'] or (market_analysis.get('broad_patterns') and market_analysis['broad_patterns']['patterns'])):
...
def handle_scanner_mode(scan_type, num_years, min_shift_score, sector_stocks, industry_stocks):
    ...
    if scan_type == "All Industries":
        ...
        st.warning("Custom selection not implemented in this section.")
        return
    
    # Create a mode-specific cache key
    current_scan_params = {
...
def main():
    ...
    try:
        ...
        if not sector_stocks or not industry_stocks:
            st.error("Failed to load stock classifications. Please check your 'stock_sectors.csv' file.")
            return
    except Exception as e:
        st.error(f"Error loading stock classifications: {str(e)}")
